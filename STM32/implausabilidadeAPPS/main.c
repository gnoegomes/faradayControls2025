...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	 ...
	 {
		 appsValue1 = HAL_ADC_GetValue(&hadc1);
	     
	 }

	 if (HAL_ADC_PollForConversion(&hadc1, HAL_MAX_DELAY) == HAL_OK)
	 {
		 appsValue2 = HAL_ADC_GetValue(&hadc1);
	 }

	 double difference = fabs(appsValue1 - appsValue2);
	 double average = (appsValue1 + appsValue2) / 2.0;
	 ...
	 if (average != 0) {
	        ...
	        // Se a média for zero, reseta
	        conditionActive = false;
	        HAL_GPIO_WritePin(LED_GPIO_Port, LED_Pin, GPIO_PIN_RESET);
	    }
	 
	 HAL_ADC_Stop(&hadc1);
	 HAL_Delay(100);		//REVER ESSE DELAY, ELIMIINAR SE NECESSARIO
